 ...
 * 
 * Gemini handles the phrasing (see insight-gemini.ts)
 */
...
export interface MealInsightSignal {
    ...
    facts: {
        ...
        caloriesPerProtein?: number // calories per gram of protein
    }
    userGoal: PrimaryGoal
    secondaryFocuses: SecondaryFocus[]
    meal: MealNutrition
    // Daily context for situational awareness
    dailyProgress?: {
        proteinMet: boolean
        carbsMet: boolean
        fatMet: boolean
...
export interface DailyInsightSignal {
    ...
    facts: {
        ...
        allGoalsMet?: boolean
        allGoalsExceeded?: boolean // User has clearly exceeded all targets
        proteinMet?: boolean
        carbsMet?: boolean
        fatMet?: boolean
        // Remaining amounts (for specific suggestions)
    ...
    mealDescriptions?: string[] // What the user ate today
}
...
export function getDailyInsightSignal(
    ...
    if (day.mealCount === 0) {
        ...
        if (allGoalsExceeded) {
            ...
            priority = 'low' // Completely done, just celebrate
        } else if (allGoalsMet) {
            facts.allGoalsMet = true
            priority = 'low' // No action needed
            // Individual macro status
