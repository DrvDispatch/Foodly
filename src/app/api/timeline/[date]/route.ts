...
export async function GET(
    ...
    try {
        ...
        const timelineMeals = meals.map((meal, index) => {
        ...
        if (timelineMeals.length > 0) {
            try {
                ...
                const mealTimes = timelineMeals.map(m => {
                    const time = new Date(`2000-01-01 ${m.time}`)
                    return time.getHours()
                })
                const avgMealHour = mealTimes.reduce((a, b) => a + b, 0) / mealTimes.length
                const morningCals = timelineMeals
                    .filter(m => new Date(`2000-01-01 ${m.time}`).getHours() < 12)
                ...
                const eveningCals = timelineMeals
                    .filter(m => new Date(`2000-01-01 ${m.time}`).getHours() >= 17)
                    .reduce((sum, m) => sum + m.calories, 0)
