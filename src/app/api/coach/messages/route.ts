    ...
    try {
        ...
        const buildMealDescription = (meal: typeof todayMeals[0]) => {
            ...
            if (meal.items && meal.items.length > 0) {
                const itemDescriptions = meal.items.map((item: any) => {
        ...
        const mealContext = todayMeals.map((m: any) => {
        ...
        const recentDaysSummary = weekMeals
            .filter((m: any) => m.mealTime.toISOString().split('T')[0] !== today)
            .slice(0, 10)
            .map((m: any) => `${m.mealTime.toISOString().split('T')[0]}: ${buildMealDescription(m)}`)
            .join('; ')
