...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const aggregatePeriod = (meals: typeof period1Meals, start: Date, end: Date) => {
            ...
            const avgCalories = Math.round(values.reduce((s: number, v: any) => s + v.calories, 0) / values.length)
            const avgProtein = Math.round(values.reduce((s: number, v: any) => s + v.protein, 0) / values.length)
            const avgCarbs = Math.round(values.reduce((s: number, v: any) => s + v.carbs, 0) / values.length)
            const avgFat = Math.round(values.reduce((s: number, v: any) => s + v.fat, 0) / values.length)

            // Calorie variability (std dev)
            const calValues = values.map((v: any) => v.calories)
            const calMean = calValues.reduce((a: number, b: number) => a + b, 0) / calValues.length
            const calVariance = calValues.reduce((sum: number, v: number) => sum + Math.pow(v - calMean, 2), 0) / calValues.length
