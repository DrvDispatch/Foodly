...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const calculateStats = (values: number[], goal: number) => {
            ...
            const sum = nonZeroValues.reduce((a: number, b: number) => a + b, 0)
            const mean = sum / nonZeroValues.length

            // Standard deviation
            const squaredDiffs = nonZeroValues.map((v: number) => Math.pow(v - mean, 2))
            const variance = squaredDiffs.reduce((a: number, b: number) => a + b, 0) / nonZeroValues.length
        ...
        const stats = {
            calories: calculateStats(dataPoints.map((d: any) => d.calories), goals.calories),
            protein: calculateStats(dataPoints.map((d: any) => d.protein), goals.protein),
            carbs: calculateStats(dataPoints.map((d: any) => d.carbs), goals.carbs),
            fat: calculateStats(dataPoints.map((d: any) => d.fat), goals.fat)
        }

        // Data confidence indicator
        const loggedDays = dataPoints.filter((d: any) => d.mealCount > 0).length
