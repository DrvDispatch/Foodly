...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const latestMealUpdate = meals.reduce((latest: any, meal: any) => {
        ...
        // Count unique days
        const uniqueDays = new Set(meals.map((m: any) => m.mealTime.toISOString().split('T')[0]))
        ...
        meals.forEach((meal: any) => {
            ...
            const itemNames = meal.items.map((i: any) => i.name).join(', ')
