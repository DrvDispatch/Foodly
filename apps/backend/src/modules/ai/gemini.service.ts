...
/**
 * Meal analysis result structure
 * Matches the structured output schema for Gemini
 */
...
const mealAnalysisSchema = {
    ...
    properties: {
        ...
        title: {
            type: 'string',
            description: 'Short, appetizing title for the meal (max 5-7 words)',
        },
        description: {
            type: 'string',
            description: 'Detailed description of the meal including all items, portions, and preparation style',
        },
        notes: {
            type: 'array',
            items: { type: 'string' },
...
/**
 * Gemini Service - Meal Analysis
 * 
 * Uses Google Gemini 3 Flash Preview for:
 * - Image-based meal recognition
 * - Nutritional estimation (macros + micros)
 * - Meal type inference from timestamp + content
 * 
 * HARD CONSTRAINT: Must use gemini-3-flash-preview model
 */
@Injectable()
export class GeminiService {
     ...
     * - Meal type inferred from image + timestamp
     * - Macro and micronutrient estimation
...
`;
        ...
        try {
            ...
            if (imageBase64) {
                ...
                const base64Data = this.cleanBase64(imageBase64);
                parts.push({
                    inlineData: {
                        data: base64Data,
                        mimeType: mimeType,
                ...
                console.log('[GeminiService] Added primary image, mimeType:', mimeType, ', base64 length:', base64Data.length);
            }

            // Add additional images with proper MIME type detection
            if (additionalImages && additionalImages.length > 0) {
                for (const img of additionalImages.slice(0, 3)) {
                    const mimeType = this.detectMimeType(img);
                    const base64Data = this.cleanBase64(img);
                    parts.push({
                        inlineData: {
                            data: base64Data,
                            mimeType: mimeType,
                ...
                console.log('[GeminiService] Added', additionalImages.length, 'additional images');
            }

            // Add text prompt last
            parts.push({ text: prompt });

            console.log(`[GeminiService] Calling ${this.MODEL_NAME} with ${parts.length - 1} image(s)`);
