...
import { ChevronDown, ChevronUp, Shield, Download, Upload, Trash2, AlertTriangle, X, FileText, LogOut } from 'lucide-react'
...
export function DataControlSection({ className }: DataControlSectionProps) {
    ...
    const handleDelete = async () => {
        ...
        setIsDeleting(true)
        try {
            // Send confirmation in request body
            await apiClient.post('/account/delete', { confirmation: deleteConfirmation })
    ...
    return (
        <div className={cn("bg-white rounded-2xl shadow-sm border border-surface-100 overflow-hidden", className)}>
            ...
            {/* Expanded content */}
            {isExpanded && (
                <div className="px-4 pb-4 space-y-4 border-t border-surface-100 dark:border-surface-700 pt-4 animate-in fade-in slide-in-from-top-2 duration-200">
                    {/* Export Data */}
                    <button
                        onClick={handleExport}
                        disabled={isExporting}
                        className="w-full flex items-center justify-between p-3 bg-surface-50 dark:bg-surface-700 rounded-xl hover:bg-surface-100 dark:hover:bg-surface-600 transition-colors"
                    >
